...
var pathParser = (basePath, index, type) => {
  if (
    [
      import_types.SignerType.ecdsa,
      import_types.SignerType.ed25519,
      import_types.SignerType.sr25519,
    ].includes(type) &&
    basePath === "//"
  ) {
...
var KeyRing = class {
  ...
  constructor(storage, locktime = 30 * 60 * 1e3) {
    ...
    this.#privkeys = {};
    this.#signers = {
      [import_types2.SignerType.secp256k1]:
        new import_signer_ethereum.default(),
      [import_types2.SignerType.ecdsa]: new import_signer_polkadot.default(
        import_types2.SignerType.ecdsa
      ),
      [import_types2.SignerType.ed25519]: new import_signer_polkadot.default(
        import_types2.SignerType.ed25519
      ),
      [import_types2.SignerType.sr25519]: new import_signer_polkadot.default(
        import_types2.SignerType.sr25519
      ),
      [import_types2.SignerType.secp256k1btc]:
        new import_signer_bitcoin.default(),
    };
  }
  async init(
    password,
    ...
    (0, import_assert.default)(
      password,
      import_types2.Errors.KeyringErrors.NoPassword
    );
    const entropy = (0, import_utils.hexToBuffer)(
      (0, import_bip39.mnemonicToEntropy)(mnemonic)
    );
  ...
  async #getPathIndex(basePath) {
    const pathIndexes =
      (await this.#storage.get(configs_default.STORAGE_KEYS.PATH_INDEXES)) ||
      {};
    if (pathIndexes[basePath] === void 0) return 0;
  ...
  async #getMnemonic(password) {
    ...
    (0, import_assert.default)(
      encrypted,
      import_types2.Errors.KeyringErrors.NotInitialized
    );
    const decryptedEntropy = await (0, import_utils.decrypt)(
      encrypted,
      password
    );
  ...
  async createKey(key) {
    (0, import_assert.default)(
      !this.#isLocked,
      import_types2.Errors.KeyringErrors.Locked
    );
  ...
  async #saveKeyRecord(keyRecord) {
    ...
    await this.#storage.set(
      configs_default.STORAGE_KEYS.KEY_INFO,
      existingKeys
    );
    const pathIndexes =
      (await this.#storage.get(configs_default.STORAGE_KEYS.PATH_INDEXES)) ||
    ...
    await this.#storage.set(
      configs_default.STORAGE_KEYS.PATH_INDEXES,
      pathIndexes
    );
  }
  async sign(msgHash, options) {
    ...
    (0, import_assert.default)(
      !Object.values(import_types2.HWwalletType).includes(options.walletType),
  ...
  async ethereumDecrypt(encryptedMessage, options) {
    ...
    return this.#signers[options.signerType].decrypt(encryptedMessage, keypair);
  }
  async getKeysObject() {
    const jsonstr = await this.#storage.get(
      configs_default.STORAGE_KEYS.KEY_INFO
    );
    if (!jsonstr) return {};
    return jsonstr;
  ...
  async renameAccount(address, newName) {
    ...
    (0, import_assert.default)(
      ...
      import_types2.Errors.KeyringErrors.AddressDoesntExists
    );
  ...
  async #getPrivateKeys(keyringPassword) {
    ...
    if (!encrypted) return {};
    const decrypted = await (0, import_utils.decrypt)(
      encrypted,
      keyringPassword
    );
