...
describe("Swap", () => {
  ...
  const yetiSwap = new Swap({
    api: web3eth,
    network: SupportedNetworkName.Ethereum,
    walletIdentifier: WalletIdentifier.yeti,
    evmOptions: {
  ...
  it("it should get all From tokens", async () => {
    await yetiSwap.initPromise;
    const fromTokens = yetiSwap.getFromTokens();
  ...
  it("it should all To tokens", async () => {
    await yetiSwap.initPromise;
    const toTokens = yetiSwap.getToTokens();
  ...
  it("it should get quote and swap", async () => {
    await yetiSwap.initPromise;
    const quotes = await yetiSwap.getQuotes({
      amount,
      fromAddress,
      fromToken,
      toToken,
    ...
    const swapOneInch = await yetiSwap.getSwap(oneInceQuote!.quote);
    expect(swapOneInch?.fromTokenAmount.toString()).to.be.eq(amount.toString());
    expect(swapOneInch?.transactions.length).to.be.eq(2);
    const swapChangelly = await yetiSwap.getSwap(changellyQuote!.quote);
