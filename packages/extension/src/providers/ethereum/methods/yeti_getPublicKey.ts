import { MiddlewareFunction, SignerType } from "@yetiwallet/types";
...
const method: MiddlewareFunction = async function (
  ...
  if (payload.method !== "yeti_getPublicKey") return next();
  else {
    const whitelistDomains = ["localhost", "raffle.yetichain.com"];
    ...
    if (!isInitialized) {
      ...
      return res(getCustomError("Yeti not initialized"));
    } else {
